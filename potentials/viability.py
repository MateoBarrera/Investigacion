...
class Pv:
  ...
  def calculate_autonomy(self):
    self.irr_mean_month = self.data.groupby(pd.PeriodIndex(self.data['Fecha'], freq="M"))['Valor'].mean()
    #Prepare data
    self.data_month, self.data_month_piv = get_data_month(self.data)
    self.__variability = calculate_variability(self.data_month_piv)
  ...
  def psh_graph(self):
    ...
    #Plot raw data
    self.data.plot(kind='line', x='Fecha', y='Valor', ax=ax1, label="Irradiance")
    ...
    ax1.hlines(y=self.irr_mean, xmin=self.data['Fecha'].min(), xmax=self.data['Fecha'].max(), colors='gray', linestyles='--',
               label="Qmean= {:.2f}".format(self.irr_mean))
    ax1.set_title('Total daily solar irradiance incident', fontdict=font)
    ax1.set_xlabel('Year', fontdict=font)
    ax1.set_ylabel('Irradiance [kWh/m^2/day]', fontdict=font)
    ax1.legend(loc='upper left')

    #Plot month data
    self.irr_mean_month.plot(kind='line', x='Fecha', y='Valor', ax=ax2, label="Irradiance")
